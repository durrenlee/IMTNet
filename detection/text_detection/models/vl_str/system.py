...
from models.base import CrossEntropySystem
...
class VL4STR(CrossEntropySystem):
    ...
    def encode(self, img: torch.Tensor):
        ...
        if self.freeze_image_backbone:
            ...
            # memory = self.clip_model.encode_image(img, cls=self.clip_cls_eot_feature, projection=self.use_share_dim)
            # replace memory with custom visual encoder output:torch.Size([batch_size, 197, 512])
    ...
    def encoder_cross_modal_feature(self, prev_logits, image_feat):
        ...
        # image_features shape: torch.Size([bs, 197, 512])
        # text_features shape: torch.Size([bs, 16, 512])
        cat_features = torch.cat([image_features, text_features], dim=1)
        # cat_features shape: torch.Size([bs, 213, 512])
        return cat_features
    ...
    def forward(self, images: Tensor, max_length: Optional[int] = None) -> Tensor:
        ...
        if self.decode_ar:
            ...
            for i in range(num_steps):
                ...
                '''
                Efficient decoding: Input the context up to the ith token. We use only one query (at position = i) at a 
        ...
        if self.use_language_model:
            ...
            if self.cross_fast_decode:
                ...
                cross_logits, cross_vec = self.cross_decode(logits, tgt_in[:, :crs_num_steps], memory,
                ...
                for i in range(crs_num_steps):
                    ...
                    p_i, cross_vec = self.cross_decode(logits, tgt_in[:, :j], memory, tgt_query=crs_pos_queries[:, i:j],
                                                       tgt_query_mask=query_mask[i:j, :j],
                                                       content_mask=content_mask[:j, :j], cross_memory=cross_memory)
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        images, labels = batch
        # tokenize labels into token sequences
        ...
        loss = 0
        loss_numel = 0
        #  n: the number of non-padding tokens in tgt_out: actual token seq length for variable labels length
        n = (tgt_out != self.pad_id).sum().item()
        # calculate loss for permute sequence modeling
        for i, perm in enumerate(tgt_perms):
            content_mask, query_mask = self.generate_attn_masks(perm)
            # decoding based on token seqs and image features
            ...
            if self.use_language_model:
                ...
                # decoding based on visual prediction logits, token seqs
                cross_logits, cross_vec = self.cross_decode(visual_logits, cross_tgt_in, memory,
    ...
    def configure_optimizers(self):
        ...
        rank_zero_info(
            "[VL4STR] The length of encoder params with and without weight decay is {} and {}, respectively.".format(
                len(enc_rest_params), len(enc_gain_or_bias_params)
            ))
        rank_zero_info(
            "[VL4STR] The length of decoder params with and without weight decay is {} and {}, respectively.".format(
                len(dec_rest_params), len(dec_gain_or_bias_params)
            ))

        print(f'self.weight_decay:{self.weight_decay}')
        print(f'self.coef_lr:{self.coef_lr}')
        print(f' self.coef_wd:{ self.coef_wd}')
        ...
        print(f'>>>>>>>>>>>>>>>>>>>>>>>>>>optimizer:{optimizer}, sched:{sched}')
