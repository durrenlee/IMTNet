...
# data_root = '/root/data/insulator_dataset/'
data_root = '/root/data/VOCdevkit/'
...
model = dict(
    ...
    backbone=dict(
        ...
        embed_dim=96,  # tiny, small
        # depths=[2, 2, 6, 3],  # tiny
        # depths=[3, 5, 8, 5],  # small
        depths=[2, 4, 6, 2],  # small
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        # data_prefix=dict(img='train2017/'), #  standard coco-format ds
        # data_prefix=dict(img='train/images/'),  # det-str dataset
...
val_evaluator = dict(
    type='VOCMetric',
    metric='mAP',
    eval_mode='area'  # '11points' is the traditional VOC 2007 evaluation, change to 'area' for VOC 2012
)
...
param_scheduler = [
    ...
    dict(
        T_max=35,  # iters/epochs from crest to trough or trough to crest
        begin=15,
        by_epoch=True,
        convert_to_iter_based=True,
        end=50,
