...
model = dict(
    ...
    neck=dict(
        type='FPN',
        in_channels=[144, 288, 576, 576],  # neck input channels == output channel of each stage in backbone
...
train_pipeline = [
    ...
    dict(
        type='Pad',
        size=(800, 1333),  # Pad the image to (800, 1333)
...
# ]
param_scheduler = [
    dict(type='LinearLR', start_factor=0.1, by_epoch=True, begin=0, end=8),
    dict(
        T_max=4,   # iters/epochs from crest to trough or trough to crest
        begin=8,
        by_epoch=True,
        convert_to_iter_based=True,
        end=12,
