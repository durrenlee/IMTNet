...
def quality_focal_loss_with_prob(pred, target, beta=2.0):
    ...
    pred_sigmoid = pred
    scale_factor = pred_sigmoid
    #  pred or pred_sigmoid or scale_factor dtype:torch.float16

    zerolabel = scale_factor.new_zeros(pred.shape)
    loss = F.binary_cross_entropy(
        pred.clone(),
        zerolabel.clone(),
        reduction='none') * scale_factor.clone().pow(beta)
    ...
    pos_label = label[pos].long()
    # positives are supervised by bbox quality (IoU) score
    pred_sigmoid[pos, pos_label] = pred_sigmoid[pos, pos_label]
    scale_factor = score[pos] - pred_sigmoid[pos, pos_label]

    with torch.cuda.amp.autocast(enabled=False):
        loss[pos, pos_label] = F.binary_cross_entropy(
            pred[pos, pos_label].to(dtype=torch.float32),
            score[pos],
            reduction='none') * scale_factor.abs().pow(beta)

    # loss.to(device=device, dtype=torch.float16)
    loss = loss.sum(dim=1, keepdim=False)
