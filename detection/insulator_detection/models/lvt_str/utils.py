...
def _get_config(experiment: str, **kwargs):
    ...
    with open(root / f'configs/model/{experiment}.yaml', 'r') as f:
...
def _get_model_class(key):
    print(f'_get_model_class, key:{key}')
    exp = 'parseq'
    if 'abinet' in key:
        ...
        exp = 'str_adapter'
    elif 'insulatordetandstr' in key:
        from insulator_detection.models.framework.insulator_det_and_recog import InsulatorDetAndReco as ModelClass
        # from insulator_detection.models.lvt_str.str_model import InsulatorSTRModel as ModelClass
        print('*****************InsulatorDetAndReco*****************')
        exp = 'insulatordetandstr'
    else:
...
def create_model(experiment: str, pretrained: bool = False, **kwargs):
    try:
        print(f'experiment in create_model:{experiment}')
...
def load_from_checkpoint(checkpoint_path: str, **kwargs):
    ...
    try:
        ...
        ModelClass, _ = _get_model_class(checkpoint_path)
        print(f'MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM')
        model = ModelClass.load_from_checkpoint(checkpoint_path, **kwargs)
    except:
