...
from models.lvt_str.utils import load_from_checkpoint, parse_model_args
...
def main():
    ...
    for fname in files:
        ...
        p = model.str_model(image).softmax(-1)
        pred, p = model.str_model.tokenizer.decode(p)
        sequence_confidence = p[0].mean().item()  # Average confidence across characters
        print(f'{fname}: {pred[0]}, sequence confidence: {sequence_confidence}')
